...
const envSchema = z.object({
  ...
  SECURITY_SESSION_SECRET: z.string().min(32, 'SECURITY_SESSION_SECRET must be at least 32 characters'),
...
export function getConfig() {
  ...
  const envWithDefaults = isTest ? {
    ...process.env,
    JWT_SECRET: process.env.JWT_SECRET || 'test_jwt_secret_minimum_32_characters_for_testing_only',
    SECURITY_SESSION_SECRET: process.env.SECURITY_SESSION_SECRET || 'test_session_secret_minimum_32_characters_testing',
  } : process.env;
  ...
  try {
    ...
    if (parsed.NODE_ENV === 'production') {
      if (!process.env.JWT_SECRET) {
        throw new ValidationError('JWT_SECRET must be explicitly set in production');
      }
      if (!process.env.SECURITY_SESSION_SECRET) {
        throw new ValidationError('SECURITY_SESSION_SECRET must be explicitly set in production');
      }
    }
